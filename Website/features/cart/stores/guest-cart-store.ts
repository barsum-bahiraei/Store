...
type GuestCartState = {
  items: GuestCartItem[];
  change: (productId: number, action: CartAction) => void;
...
export const useGuestCartStore = create<GuestCartState>()(persist((set) => ({
  items: [],
  change: (productId, action) => set((state) => {
    ...
    if (!item) return action === "increase" ? { items: [...state.items, { productId, count: 1 }] } : state;
    return { items: state.items.map((entry) => entry.productId === productId
      ? { productId, count: entry.count + (action === "increase" ? 1 : -1) } : entry) };
  }),
