...
export function HeroSlider() {
  ...
  return (
    <section aria-label="Featured collections" className="relative w-full overflow-hidden bg-secondary">
      <Swiper
        ...
        a11y={{ prevSlideMessage: "Previous promotion", nextSlideMessage: "Next promotion" }}
        ...
        {heroSlides.map((slide, index) => (
          <SwiperSlide key={slide.title}>
            <article className="grid min-h-[31rem] bg-secondary text-secondary-foreground lg:min-h-[29rem] lg:grid-cols-[0.72fr_1.28fr]">
              <div className="relative z-10 flex items-center px-5 pb-14 pt-9 sm:px-10 lg:px-12 lg:py-14 xl:pl-[max(3rem,calc((100vw-80rem)/2))] xl:pr-16">
      ...
      <div className="pointer-events-none absolute right-5 top-5 z-20 hidden gap-2 sm:flex lg:right-8 lg:top-1/2 lg:-translate-y-1/2 lg:flex-col">
        <button type="button" onClick={() => swiperRef.current?.slidePrev()} aria-label="Previous promotion" className="pointer-events-auto grid size-11 place-items-center rounded-full border border-border bg-surface/90 text-foreground outline-none backdrop-blur-sm transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-ring">
          <span className="material-symbols-rounded" aria-hidden="true">arrow_back</span>
        </button>
        <button type="button" onClick={() => swiperRef.current?.slideNext()} aria-label="Next promotion" className="pointer-events-auto grid size-11 place-items-center rounded-full border border-border bg-surface/90 text-foreground outline-none backdrop-blur-sm transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-ring">
